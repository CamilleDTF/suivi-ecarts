...
# Copier .env.example vers .env et renseigner DATABASE\_URL + AUTH\_SECRET
cp .env.example .env
...
|-|-|
|`DATABASE\_URL`|Chaîne de connexion PostgreSQL (`postgresql://user:password@host:port/db?schema=public`)|
|`AUTH\_SECRET`|Secret utilisé pour signer les sessions (`npx auth secret` en génère un)|
...
pour démarrer. Récupérer la chaîne de connexion (`DATABASE\_URL`).
2. **Déployer sur Vercel** :

   * Importer ce dépôt dans [Vercel](https://vercel.com/new).
   * Renseigner les variables d'environnement `DATABASE\_URL` et `AUTH\_SECRET`
(génère `AUTH\_SECRET` avec `npx auth secret`, à faire une seule fois).
...
déploiement (depuis ta machine, avec `DATABASE\_URL` pointant vers la prod) :

```bash
   DATABASE\_URL="<url de prod>" npx prisma migrate deploy
   DATABASE\_URL="<url de prod>" npx prisma db seed
   ```
